...
data_day3 <- read_csv(file = "data/data_day2.csv")
head(data_day3)

# Plot all CBC data
data_day3 %>%
  distinct(Counts, CBC, Sample.ID, .keep_all = TRUE) %>%
  ggplot(aes(x = CBC, y = Counts, color = CBC)) + 
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

# Challenge
# Produce boxplot with FracCD45.Neutrophil by HighestCare,
...
WBC$severity <- factor(WBC$severity, 
levels = c("Mild","Moderate","Severe", "Neg"))
...
WBC %>%
  ...
  geom_point(aes(fill = severity), alpha = 0.5, pch = 21, color = "black") +
  theme_bw() +
  labs(x = NULL, y = "Absolute Counts", 
       title = "White Blood Cells (WBC)") +
  scale_fill_manual(labels = c("Mild COVID-19", "Moderate COVID-19",
                               "Severe COVID-19",
                               "Hospitalized COVID-19 Negative"), 
                    values = c("cyan", "blue","red", "gray")) +
...
# Recreate 10D
levels(as.factor(data_day3$CYTOF))
...
# Class
class(data_day3_10d$tocilizumab.EndDate)
class(data_day3_10d$tocilizumab.StartDate)
class(data_day3_10d$drawDate)

# conditional mutate
data_day3_10d <- data_day3_10d %>%
  mutate(tocilizumab.timing = case_when(
...
data_day3_10d <- data_day3_10d %>% mutate(CYTOF_percent = Fraction*100)

class(data_day3_10d$tocilizumab.timing)
levels(as.factor(data_day3_10d$tocilizumab.timing))

# make factor
data_day3_10d$tocilizumab.timing <- factor(data_day3_10d$tocilizumab.timing, 
                                           levels = c("before_tocilizumab", "after_tocilizumab"),
                                           labels = c("Before Tocilzumab", "After Tocilizumab"))
levels(data_day3_10d$tocilizumab.timing)

# Make plot
neutrophil_plot <- data_day3_10d %>%
  filter(CYTOF == "FracCD45.Neutrophil") %>%
  distinct(eventId, .keep_all = TRUE) %>%
  ggplot(aes(x = tocilizumab.timing, y = CYTOF_percent, color = tocilizumab.timing)) +
  geom_boxplot() +
  geom_point(aes(color=tocilizumab.timing)) +
  geom_line(aes(group = covidId) , color = "grey") + 
  theme_classic() + 
  scale_color_manual(name = NULL, values = c("#fe5750", "#00c7ff")) +
  theme(axis.text.x = element_blank(), axis.ticks = element_blank()) + 
  labs(x = NULL, y = "% of CD45", title = "Neutrophils")

neutrophil_plot
  
write.csv(data_day3_10d, "data/data_day3.csv")

### DAY 4 ####
library(tidyverse)

data_day4 <- read_csv(file = "data/data_day3.csv")
...
# export plot
ggsave(CD4_plot, filename = "figures/CD4_plot.pdf", device = "pdf",
       height = 5, width = 5)
...
combined_plots <- ggarrange(neutrophil_plot, CD4_plot, nrow = 1,
                            labels = c("A", "B"), width = c(1, 0.8))
combined_plots

ggarrange(combined_plots, legend, nrow = 2, ncol = 1, height = c(1, 0.5))

ggsave(combined_plots, filename = "figures/combined_plots.pdf",
       device = "pdf", height = 3, width = 10)

### PCA plotting
library(PCAtools)

data <- read_csv(file = "data/Bolouri_2021_subset.csv")
head(data)

data_subset <- data %>% select(Sample.ID, CBC.White.Blood.Cell.Count,
                               CBC.Absolute.Monocytes,
                               CBC.Absolute.Neutrophils,
                               CBC.Absolute.Lymphocytes) %>% drop_na()
# check class
str(data_subset)

# Make Sample.ID rownames
data_subset_rownames <- data_subset %>% 
  column_to_rownames(var = "Sample.ID")
rownames(data_subset_rownames)

View(data_subset_rownames)

# transpose and make matrix
data_subset_mat <- as.matrix(t(data_subset_rownames))
head(data_subset_mat)

# create metadata
metadata_subset <- data %>% select(Sample.ID, sex, age, severity) %>% 
  filter(Sample.ID %in% data_subset$Sample.ID) %>%
  column_to_rownames(var = "Sample.ID") 
View(metadata_subset)
View(data_subset_mat)

metadata_subset[is.na(metadata_subset$severity),]

# check colnames and rownames
all(colnames(data_subset_mat) == rownames(metadata_subset))

# run PCA
pca <- PCAtools::pca(data_subset_mat, metadata = metadata_subset)
pca

# Plot PCA
pca_severity <- PCAtools::biplot(pca, colby = "severity",
                                 colLegendTitle = "Severity",
                                 legendPosition = "right")
pca_severity

## Challenge
# Plot PCA by sex and export the plot as a pdf

# Quick heatmap
library(ComplexHeatmap)
ComplexHeatmap::Heatmap(data_subset_mat)
